...
import pytest
from mklists.run import return_filename2datalines_dict_after_applying_rules
...
def test_return_filename2datalines_dict_after_applying_rules_no_rules_specified_either():
    ...
    lines = ["NOW Summer\n", "LATER Winter\n"]
    with pytest.raises(SystemExit):
        return_filename2datalines_dict_after_applying_rules(rules, lines)


def test_return_filename2datalines_dict_after_applying_rules_no_data_specified():
    """Not passing data to return_filename2datalines_dict_after_applying_rules raises SystemExit."""
    with pytest.raises(SystemExit):
        return_filename2datalines_dict_after_applying_rules(
...
def test_return_filename2datalines_dict_after_applying_rules_correct_result():
    """return_filename2datalines_dict_after_applying_rules works as it should."""
    rules = [Rule(0, "i", "a.txt", "b.txt", 0)]
    lines = ["two ticks\n", "an ant\n", "the mite\n"]
    mdict = {"a.txt": ["an ant\n"], "b.txt": ["two ticks\n", "the mite\n"]}
    return_filename2datalines_dict_after_applying_rules(rules, lines) == mdict
