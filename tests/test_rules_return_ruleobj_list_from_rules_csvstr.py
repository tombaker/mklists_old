...
from mklists.rules import Rule, _return_ruleobj_list_from_rules_csvstr
...
@pytest.mark.skip
def test_run_return_ruleobj_list_from_rules_csvstr(tmpdir):
    """@@@Docstring"""
    expected = TEST_RULEOBJ_LIST
    real = _return_ruleobj_list_from_rules_csvstr(csvstr=TEST_RULES_YAMLSTR)
    assert real == expected


@pytest.mark.skip
def test_run_return_ruleobj_list_from_rules_csvstr_exception(tmpdir):
    """@@@Docstring"""
    # 2019-09-30: Problem has something to do with testing for
    # wrong exception raised
    with pytest.raises(NoRulesError):
        _return_ruleobj_list_from_rules_csvstr(csvstr=None)
