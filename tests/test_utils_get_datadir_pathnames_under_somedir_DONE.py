...
from mklists.utils import get_datadir_pathnames_under_somedir


def test_get_datadir_pathnames_under_somedir(tmpdir):
    ...
    assert get_datadir_pathnames_under_somedir(tmpdir) == expected


def test_get_datadir_pathnames_under_somedir_excluding_hiddendirs(tmpdir):
    """List data directories found under project root."""
    tmpdir.join(CONFIG_YAMLFILE_NAME).write("config stuff")
    tmpdir.join(RULE_YAMLFILE_NAME).write("some rules")
