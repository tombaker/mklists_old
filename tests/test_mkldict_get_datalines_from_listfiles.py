...
import os
import pytest
from mklists.makelists import get_datalines_list_from_listfiles
...
def test_get_lines(tmpdir):
    ...
    assert get_datalines_list_from_listfiles(["foo", "bar"]) == expected_result
...
def test_get_lines_blank_lines_found(tmpdir):
    ...
    with pytest.raises(SystemExit):
        get_datalines_list_from_listfiles(["foo", "bar"])
...
def test_get_lines_non_utf8_found(tmpdir):
    ...
    with pytest.raises(SystemExit):
        get_datalines_list_from_listfiles(["foo", "bar.pickle"])
