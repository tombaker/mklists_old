...
def test_get_lines(tmpdir):
    ...
    assert get_dataline_list_from_listfiles(["foo", "bar"]) == expected_result
...
def test_get_lines_blank_lines_found(tmpdir):
    ...
    with pytest.raises(SystemExit):
        get_dataline_list_from_listfiles(["foo", "bar"])
...
def test_get_lines_non_utf8_found(tmpdir):
    ...
    with pytest.raises(SystemExit):
        get_dataline_list_from_listfiles(["foo", "bar.pickle"])
