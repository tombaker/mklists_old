...
from mklists.utils import find_datadirs_under_project_rootdir


def test_find_datadirs_under_project_rootdir(tmpdir):
    ...
    assert find_datadirs_under_project_rootdir(tmpdir) == expected


def test_find_datadirs_under_project_rootdir_with_localrules(tmpdir):
    """List data directories found under project root."""
    tmpdir.join(CONFIGFILE_NAME).write("config stuff")
    tmpdira = tmpdir.mkdir("a")
