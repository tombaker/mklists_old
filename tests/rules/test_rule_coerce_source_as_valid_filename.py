...
def test_confirm_source_is_valid_filename(tmp_path):
    """Field 3 (source) must be a valid filename."""
    os.chdir(tmp_path)
    rule_obj = Rule(1, "NOW", "a", "b", 2)
    rule_obj._confirm_source_is_valid_filename()
    assert isinstance(rule_obj.source, str)
    assert rule_obj.source == "a"


def test_confirm_source_is_valid_filename_raise_exception_given_bad_string():
...
def test_confirm_source_is_valid_filename_raise_exception_given_none():
