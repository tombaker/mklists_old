...
def test_confirm_target_is_valid_filename():
    """Field 4 (target) must be a valid filename."""
    rule_obj = Rule(1, "NOW", "a", "b", 2)
    rule_obj._confirm_target_is_valid_filename()
    assert isinstance(rule_obj.target, str)
    assert rule_obj.target == "b"


def test_confirm_target_is_valid_filename_raise_exception_given_bad_string():
...
def test_confirm_target_is_valid_filename_raise_exception_given_none():
