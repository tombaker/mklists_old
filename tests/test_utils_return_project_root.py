...
def test_return_project_rootdir_name_while_in_rootdir(tmpdir):
    """Find root directory while in root directory."""
    tmpdir.join(CONFIGFILE_NAME).write("config stuff")
    os.chdir(tmpdir)
    assert CONFIGFILE_NAME in os.listdir(return_project_rootdir_name())


def test_return_project_rootdir_name_while_in_subdir_one_deep(tmpdir):
...
def test_return_project_rootdir_name_while_in_subdir_two_deep(tmpdir):
...
def test_return_project_rootdir_name_while_in_subdir_three_deep(tmpdir):
...
def test_not_return_project_rootdir_name_given_missing_link(tmpdir):
