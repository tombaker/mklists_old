...
def explain(**sets):
    ...
    if sets["globalrules"]:
        print(
            f"Global rule file {repr(sets['globalrules'])} "
    ...
    if sets["urlify"]:
        print(
            f"'urlify' option is ON, so TXT files will be copied, "
            "converted into HTML,"
        )
        if sets["urlify_dir"]:
    ...
    if sets["backup"]:
        ...
        if backup_dir_timestamped:
            ...
            print(
                f"...where the {repr(sets['backup_depth'])} most "
                "recent backups will be kept."
            )
        else:
            print(
                "...however the command will fail "
    ...
    if sets["files2dirs"]:
        ...
        print(
            f"'files2dirs' option not set in "
